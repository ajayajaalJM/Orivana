...
export default function MemberLoginPage() {
    ...
    fetch("/api/shopify/status")
      ...
      .then((data) =>
        setShopifyStatus({
          ...
          customerAccount: Boolean(data.customerAccount),
        })
      )
      .catch(() => setShopifyStatus({ storefront: false, customerAccount: false }));
  }, []);
  ...
  const showDemoLogin = !shopifyStatus.customerAccount;
      ...
      <Section className="page-top">
        <Container>
          <div className="mx-auto max-w-md">
            ...
            {shopifyStatus.customerAccount ? (
              <div className="space-y-8">
                <Body className="text-center text-sm text-[var(--color-muted)]">
                  Sign in securely with your Orivana account. You&apos;ll be redirected to complete
                  verification, then return to the Inner Circle.
                </Body>
                <Button
                  type="button"
                  variant="primary"
              ...
              <>
                {showDemoLogin && (
                  <Body className="mb-8 text-center text-sm text-[var(--color-muted)]">
                    Demo accounts: member@orivana.com · chef@orivana.com · trade@orivana.com
                    (password: member)
                  </Body>
                )}
            ...
            <Body className="mt-8 text-center text-sm">
              Not yet a member?{" "}
              <Link href="/membership/join" className="text-[var(--color-sea)] hover:underline">
